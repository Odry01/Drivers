...
void SSD1306_DRIVER_Send_Page_Buffer(uint8_t I2C_ADDRESS)
{
    DRV_I2C_WriteTransferAdd(
        ssd1306_driverData.I2C_HANDLE,
        I2C_ADDRESS,
        (void*)ssd1306_driverData.I2C_DATA_TRANSMIT,
        SSD1306_PAGE_TX_SIZE,
        &ssd1306_driverData.I2C_TRANSFER_HANDLE);
}

void SSD1306_DRIVER_Select_Page(uint8_t I2C_ADDRESS, uint8_t PAGE_NUMBER)
{
    // Page address command: 0xB0 | page (0-7)
    SSD1306_DRIVER_Send_Command(I2C_ADDRESS, (uint8_t)(0xB0 | (PAGE_NUMBER & 0x07)));
    // Reset column address lower nibble to 0
    SSD1306_DRIVER_Send_Command(I2C_ADDRESS, SSD1306_CMD_SET_LOWER_COLUMN);
    // Reset column address higher nibble to 0
    SSD1306_DRIVER_Send_Command(I2C_ADDRESS, SSD1306_CMD_SET_HIGHER_COLUMN);
    // Note: three transfers are now queued. The Harmony I2C driver will
    // execute them in order. The transfer handle points to the last one.
}
